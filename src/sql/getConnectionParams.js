...
const getRDSEndpoint = async (rdsClient, environment, clusterId) => {
  const clusterIdentifier = `aurora-cluster-${environment}-${clusterId}`;
...
const getConnectionParams = async (environment, clusterId) => {
  ...
  const endpoint = await getRDSEndpoint(rdsClient, environment, clusterId);
