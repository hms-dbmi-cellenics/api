...
const pipelineErrorController = require('../../../src/api.v2/controllers/pipelineErrorController');

jest.mock('../../../src/api.v2/controllers/pipelineErrorController', () => ({
...
describe('Pipeline error routes tests', () => {
  beforeEach(async () => {
    const mockApp = await expressLoader(express());
    app = mockApp.app;
  });

  it('Sending a QC error returns in a successful response', async (done) => {
    pipelineErrorController.handleResponse.mockImplementationOnce((req, res) => {
      res.json(OK());
